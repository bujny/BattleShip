...
int CompGreedyStrategy::random(int i){
int r=rand()%i;
return r;
}

void CompGreedyStrategy::placeShips(shared_ptr<Ship> myShips[3]){
int xy[2];
...
void CompGreedyStrategy::makeMove(shared_ptr<Ship> myShips[3]){
int n;
int xy[2];
bool emptyMove=true;
...
if(myShips[2]->canTakeShot() && !myShips[2]->isPause()) n=2;
else if(myShips[1]->canTakeShot() && !myShips[1]->isPause()) n=1;
else if(myShips[2]->canTakeShot()) n=2;
else if(myShips[1]->canTakeShot()) n=1;
else n=0;
